...
class DummyEmbeddingFunction:
	"""Minimal embedding function to satisfy Chroma without external deps.
	Matches interface: __call__(self, input: Documents) -> Embeddings
	"""
...
class VectorStore:
	def __init__(self, persist_directory: str = None):
		...
		# Initialize a dummy embedding function to avoid runtime OpenAI dependency
		self.embedding_function = DummyEmbeddingFunction(dimension=1536)
		...
		# Create collections for each industry (attach embedding_function)
	...
	def _add_to_collection(self, collection, chunks: List[ProcessedChunk]) -> None:
		...
		# Add to collection (embedding_function on collection will embed)
		collection.add(
			documents=documents,
			metadatas=metadatas,
			ids=ids
